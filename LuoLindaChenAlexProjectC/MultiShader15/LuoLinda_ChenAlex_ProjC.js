...
// part3Box = new VBObox3();     // all the stuff for top hat fish
// part4Box = new VBObox4();     // octopus
// part5Box = new VBObox5();     //Stingray
// part6Box = new VBObox6();     //Kelp
// For animation:---------------------
...
var g_angleRate1 =  5.0;        // rotation angle rate, degrees/sec
...
var sideangle = 0;
var sideanglerate = 10.0;
var tailangle = 0;
var tailanglerate = 35.0;
var rocking = 0;
var rockingrate = 10;
//OCTOPUS:
var g_angle03 = 5;                  // initial rotation angle
var g_angle03Rate = 15.0;           // rotation speed, in degrees/second 
var g_angle03Min = 0;                  // initial rotation angle
var g_angle03Max = 100;                // initial rotation angle              
...
var blinnLighting = 0;
var lightPos = new Float32Array(3);
lightPos
var lightAmb = new Float32Array(3);
var lightDif = new Float32Array(3);
var lightSpec = new Float32Array(3);
var lightOn = true;
var g_canvas = document.getElementById('webgl'); 
lightPos.set([document.getElementById("lightPosX").value, 
              document.getElementById("lightPosY").value, 
              document.getElementById("lightPosZ").value]);
lightAmb.set([document.getElementById("lightAmbR").value, 
              document.getElementById("lightAmbG").value, 
              document.getElementById("lightAmbB").value]);
lightDif.set([document.getElementById("lightDifR").value, 
              document.getElementById("lightDifG").value, 
              document.getElementById("lightDifB").value]);
lightSpec.set([document.getElementById("lightSpecR").value, 
              document.getElementById("lightSpecG").value, 
              document.getElementById("lightSpecB").value]);
...
var canvasH = document.body.clientHeight * .7;

var imageAspect = canvasW/canvasH;
function resizeCanvas(){
  g_canvasID.width = window.innerWidth-16;
  g_canvasID.height = window.innerHeight * .7;

  canvasW =  g_canvasID.width;
  canvasH = g_canvasID.height;
  ...
  gl.clearColor(0.3, 0.5, 0.8, 1);	  // RGBA color for clearing <canvas>
...
//=============================================================================
  ...
  // g_canvasID.width  = document.body.clientWidth;
  // g_canvasID.height = document.body.clientHeight * .7;
  g_canvasID.width = window.innerWidth-16;
  g_canvasID.height = window.innerHeight * .7;
  canvasW =  g_canvasID.width;
  canvasH = g_canvasID.height;
...
// Find new values for all time-varying parameters used for on-screen drawing
  ...
  if(g_posNow1 > g_posMax1) {   // above the max?
    ...
    rocking = rocking + (rockingrate * elapsedMS) / 1000.0;
	  if(rocking >  10.0 ) 
	  {
		  rockingrate = -rockingrate;
		  rocking = 10.0;
	  }
	  if(rocking <  -10.0){
		rockingrate = -rockingrate;
	  ...
	  tailangle = tailangle + (tailanglerate * elapsedMS) / 1000.0;
	  if(tailangle >  30.0 ) 
	  {
		  tailanglerate = -tailanglerate;
		  tailangle = 30.0;
	  }
	  if(tailangle <  -30.0){
		tailanglerate = -tailanglerate;
	  ...
	  sideangle = sideangle + (sideanglerate * elapsedMS) / 1000.0;
	  if(sideangle >  30.0) 
	 {
		sideanglerate = -sideanglerate;
		sideangle = 30.0;
	 } 
	...
	g_angle03 = g_angle03 + (g_angle03Rate * elapsedMS) / 1000.0; // rate in degrees/sec
	sideangle = sideangle + (sideanglerate * elapsedMS) / 1000.0;
	  if(g_angle03 >  g_angle03Max) 
	 {
		g_angle03Rate = -g_angle03Rate;
		g_angle03 = g_angle03Max;
	 } 
	  
	  if(g_angle03 <  g_angle03Min){
		g_angle03Rate = -g_angle03Rate;
		g_angle03 = g_angle03Min;
...
var b4Wait = b4Draw - g_lastMS;
  ...
  //   part5Box.switchToMe();  // Set WebGL to render from this VBObox.
  // 	part5Box.adjust();		  // Send new values for uniforms to the GPU, and
  //   part5Box.draw();        // draw our VBO's contents using our shaders.
    
  //   part6Box.switchToMe();  // Set WebGL to render from this VBObox.
  // 	part6Box.adjust();		  // Send new values for uniforms to the GPU, and
  // 	part6Box.draw();        // draw our VBO's contents using our shaders.
...
// Called when user presses HTML-5 button 'Show/Hide VBO2'.
  ...
  function lightingToggle() {
    //=============================================================================
    // Called when user presses HTML-5 button 'Show/Hide VBO2'.
    var lightingState = blinnLighting == 1;
    if(lightingState){
      blinnLighting= 0;
      document.getElementById("lighting").innerText= "Change to BlinnPhong";
    }
    else{
      blinnLighting = 1;
      document.getElementById("lighting").innerText= "Change to Phong";
    }
  }
  function lightSubmit() {
    if (document.getElementById("lightOn").checked){
      lightPos.set([document.getElementById("lightPosX").value, 
                    document.getElementById("lightPosY").value, 
                    document.getElementById("lightPosZ").value]);
      lightAmb.set([document.getElementById("lightAmbR").value, 
